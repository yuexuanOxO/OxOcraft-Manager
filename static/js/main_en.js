...
function handleBackendDisconnected() {
    ...
    if (statusText) {
        statusText.textContent = "The management interface is interrupted.";
    }
...
async function updateStatus() {
    try {
        ...
        if (!data.online && wasServerOnline) {
            ...
            lastLogText = "";
            if (logBox) {
                logBox.textContent = "The server has not yet started.";
            }
...
async function updatePlayers() {
    try {
        ...
        if (!data.success || !data.players || data.players.length === 0) {
            playersList.innerHTML = "<div class='no-player'>There are currently no players online.</div>";
            return;
        }
        ...
        data.players.forEach(player => {
            ...
            kickBtn.type = "button";
            kickBtn.textContent = "Kick out of server";
...
document.addEventListener("click", async (event) => {
    ...
    if (menuItem) {
        ...
        if (action === "kick") {
            const ok = confirm(`Confirm to kick player ${player} ？`);
            if (!ok) return;
...
function formatDeathTime(value) {
    ...
    return `Time of death：${y}/${m}/${d} ${hh}:${mm}`;
}
...
function renderDeathRecordPage() {
    if (!deathRecords.length) {
        ...
        document.getElementById("deathPlayerName").textContent = "There are currently no reported deaths.";
        document.getElementById("deathPageInfo").textContent = "Page 0 of 0";
        document.getElementById("deathText").textContent = "No information is currently available.";
    ...
    document.getElementById("deathPageInfo").textContent =
        `Page ${currentDeathPage + 1} of ${deathRecords.length} `;
